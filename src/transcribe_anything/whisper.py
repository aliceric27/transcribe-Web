...
def get_environment() -> IsoEnv:
    ...
    force_cuda_env = os.environ.get("TRANSCRIBE_ANYTHING_FORCE_CUDA_ENV", "0") == "1"
    needs_extra_index = not IS_MAC and (has_nvidia_smi() or force_cuda_env)
    ...
    content_lines.append("[tool.uv.extra-build-dependencies]")
    content_lines.append('openai-whisper = ["setuptools<81"]')
