...
# Create persistent directories for uploads and results
RUN mkdir -p /app/data/uploads /app/data/results

# Pre-initialize GPU dependencies to reduce startup time
RUN /bin/bash /app/entrypoint.sh --only-check-shared-libs && \
    TRANSCRIBE_ANYTHING_FORCE_CUDA_ENV=1 transcribe-anything-init-cuda && \
